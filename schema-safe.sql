-- =====================================================
-- SAFE SUPABASE SCHEMA - NO INFINITE RECURSION
-- =====================================================
-- This schema is designed to be safe and avoid infinite recursion
-- in RLS policies. It can be run multiple times safely.
...
-- =====================================================
-- CREATE SAFE RLS POLICIES (NO INFINITE RECURSION)
-- =====================================================

-- Categories policies (simple and safe)
...
-- Products policies (simple and safe)
...
-- User profiles policies (simple and safe)
...
CREATE POLICY "User profiles are manageable by admins" ON public.user_profiles
    FOR ALL USING (auth.role() = 'service_role');

-- Cart items policies (simple and safe)
CREATE POLICY "Users can manage their own cart items" ON public.cart_items
    FOR ALL USING (auth.uid() = user_id);

-- Orders policies (simple and safe)
...
-- Order items policies (simple and safe)
...
-- User addresses policies (simple and safe)
CREATE POLICY "Users can manage their own addresses" ON public.user_addresses
    FOR ALL USING (auth.uid() = user_id);

-- Coupons policies (simple and safe)
CREATE POLICY "Coupons are viewable by everyone" ON public.coupons
    FOR SELECT USING (is_active = true AND (valid_until IS NULL OR valid_until > NOW()));

CREATE POLICY "Coupons are manageable by admins" ON public.coupons
    FOR ALL USING (auth.role() = 'service_role');

-- Wishlist policies (simple and safe)
CREATE POLICY "Users can manage their own wishlist" ON public.wishlist
    FOR ALL USING (auth.uid() = user_id);

-- Product reviews policies (simple and safe)
...
CREATE POLICY "Product reviews are manageable by admins" ON public.product_reviews
    FOR ALL USING (auth.role() = 'service_role');

-- Notifications policies (simple and safe)
CREATE POLICY "Users can manage their own notifications" ON public.notifications
    FOR ALL USING (auth.uid() = user_id);

-- Site settings policies (simple and safe)
...
-- This schema is now safe and ready to use!
-- No infinite recursion issues
-- All policies are simple and safe

-- =====================================================
-- PRODUCTION SCHEMA - COMPLETE & SAFE
-- =====================================================
-- This schema is production-ready and safe to run multiple times
-- Includes admin user setup and complete data
...
-- =====================================================
-- CREATE SIMPLE RLS POLICIES (NO INFINITE RECURSION)
-- =====================================================
...
CREATE POLICY "Categories are manageable by service role" ON public.categories
...
CREATE POLICY "Products are manageable by service role" ON public.products
    FOR ALL USING (auth.role() = 'service_role');
...
CREATE POLICY "User profiles are manageable by service role" ON public.user_profiles
    FOR ALL USING (auth.role() = 'service_role');
...
CREATE POLICY "Orders are manageable by service role" ON public.orders
    FOR ALL USING (auth.role() = 'service_role');
...
CREATE POLICY "Coupons are manageable by service role" ON public.coupons
    FOR ALL USING (auth.role() = 'service_role');
...
CREATE POLICY "Product reviews are manageable by service role" ON public.product_reviews
    FOR ALL USING (auth.role() = 'service_role');
...
CREATE POLICY "Site settings are manageable by service role" ON public.site_settings
    FOR ALL USING (auth.role() = 'service_role');
...
-- This schema is now production-ready and safe!
